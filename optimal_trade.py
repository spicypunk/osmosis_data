...
osmosis_price = 0.8489619354123147
bid_price = 0.885
...
sp_goal = bid_price
sp_now = osmosis_price = 0.8489619354123147
...
def find_optimal_size(min, max):
    ...
    if calculate_profit(testing_size - 500) < calculate_profit(testing_size) and calculate_profit(testing_size + 500) < calculate_profit(testing_size):
        return testing_size
    else: 
        if calculate_profit(testing_size - 500) > calculate_profit(testing_size) and calculate_profit(testing_size + 500) < calculate_profit(testing_size):
            return find_optimal_size(min, testing_size-500)
        elif calculate_profit(testing_size - 500) < calculate_profit(testing_size) and calculate_profit(testing_size + 500) > calculate_profit(testing_size):
            return find_optimal_size(testing_size + 500, max)




def main():
    maxAmountIn = etherToNum(balanceIn * (((sp_goal/sp_now)**(1 / 2)) - 1))
    # print(maxAmountIn)
    trade_size = find_optimal_size(0, maxAmountIn)
    print(f"the optimal trade size is {trade_size} USDC into Osmosis")
    profit = calculate_profit(trade_size)
    print(f"the profict of the arb is {profit}")
